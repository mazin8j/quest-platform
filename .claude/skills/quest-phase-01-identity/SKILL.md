...
- Phase 00 has passed an independent phase-gate audit (`docs/governance/PHASE_GATE_AUDIT_*.md` with PASS or PASS WITH CONDITIONS) and `PROGRESS.md` records it.
...
- a new user can securely onboard, select interests, create/update a profile, sign out/in and exercise privacy controls with automated unit/integration/E2E tests; rate limiting and abuse controls on auth endpoints; threat model and privacy classification recorded; `identity.account.*` events defined; account export/deletion design documented.
