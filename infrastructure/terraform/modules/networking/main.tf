...
# Single NAT gateway by default (cost-aware for dev); set nat_gateway_count = az_count for HA.
...
# VPC endpoints keep S3/ECR/logs traffic off the NAT gateway (cost + security).
...
resource "aws_cloudwatch_log_group" "flow" {
  count             = var.enable_flow_logs ? 1 : 0
  name              = "/quest/${var.name}/vpc-flow-logs"
